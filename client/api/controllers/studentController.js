...
export class StudentController {
  static login(req, res) {
    try {
      ...
      // Check if ANY OTHER student already used this PRN in the entire college
      ...
      // CASE 1: RETURNING BOUND STUDENT
      ...
      // CASE 2: NEW STUDENT INITIAL REGISTRATION
      if (!student) {
        student = {
      ...
      // RECORD AUDIT LOG FOR HOD INSPECTION
      db.addLog({
      ...
      // Safe fallback response so student is never blocked
  ...
  static submitAttendance(req, res) {
    try {
      const { sessionId, studentId, rollNo, studentName, division, department, batch, enteredPin, deviceId, fingerprint, timestamp } = req.body || {};
